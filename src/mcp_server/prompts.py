...
def excel_quickstart(file_path: str, sheet_name: str = "Sheet1") -> str:
    """Create a quickstart prompt for creating and formatting a new Excel workbook.

    Return a multi-step instruction string that directs the MCP agent to create
    an `.xlsx` workbook at `file_path`, populate a headered table, apply header
    and column formatting, freeze the header row, and auto-fit columns.

    Args:
        file_path (str): Destination path for the new workbook.
        sheet_name (str): Initial sheet name. Defaults to "Sheet1".

    Returns:
        str: A multi-line prompt string for MCP tool execution.
    """
    return f"""\
...
def excel_data_analysis(
    ...
    """Build a prompt for exploratory data analysis on a sheet.

    The returned prompt instructs the MCP agent to profile data, compute column
    statistics, run filtering/aggregation/duplicates checks and produce a concise
    summary. Behaviour varies with `analysis_type`.

    Args:
        file_path (str): Path to the workbook to analyse.
        sheet_name (str): Target sheet name. Defaults to "Sheet1".
        analysis_type (str): Which phases to run (e.g. "all", "profile", "aggregate").
...
def excel_data_cleaning(
    ...
    """Create a prompt that runs a safe data-cleaning pipeline and writes cleaned output.

    The prompt sequences validation, preview, and application of cleaning steps
    (trim, normalise, fill missing), plus deduplication and export. It emphasises
    preview-first semantics to avoid accidental overwrites.

    Args:
        input_path (str): Source workbook or CSV path (read-only).
        output_path (str): Destination workbook path for cleaned results.
        sheet_name (str): Sheet to operate on. Defaults to "Sheet1".
        fill_strategy (str): Strategy for filling missing values (e.g. "ffill").
...
def excel_chart_builder(
    ...
    """Generate a prompt to build and style a chart from a specified data range.

    The returned prompt explains how to validate the data range, create the
    requested chart type (including combo charts), set axes, data labels and
    legend, and anchor the chart in a clear target cell.

    Args:
        file_path (str): Path to the workbook.
        data_range (str): Excel range (including header row) used for series.
        sheet_name (str): Sheet containing the data. Defaults to "Sheet1".
        chart_type (str): Chart type (e.g. "bar", "line"). Defaults to "bar".
        title (str): Chart title. Defaults to "Chart".
...
def excel_report_builder(
    ...
    """Construct a prompt to assemble a multi-sheet, print-ready report workbook.

    The prompt guides aggregation of statistics, creation of a cover page,
    summary and charts sheets, consistent styling, and print setup steps.

    Args:
        output_path (str): Destination path for the report workbook.
        source_path (str): Source workbook containing data to summarise.
        source_sheet (str): Source sheet name. Defaults to "Sheet1".
        report_title (str): Title to use on the cover sheet.
...
def excel_financial_model(
    ...
    """Create a prompt to build a multi-sheet financial model (amortisation, DCF, ratios).

    The prompt describes calling domain tools (loan amortisation, DCF analysis,
    ratio calculators) and writing formatted outputs to named sheets.

    Args:
        file_path (str): Destination workbook path.
        model_type (str): Which parts of the model to build (e.g. "full", "amortisation").
        principal (float): Principal amount used for amortisation calculations.
        annual_rate (float): Annual rate as a decimal (e.g. 0.05 for 5%).
        term_years (int): Term length in years.
...
def excel_pivot_etl(
    ...
    """Return a prompt to create pivot tables and run ETL transforms on a sheet.

    The prompt covers pivot creation, unpivot operations, merges and computed
    columns; it instructs the agent to persist pivot definitions and to set an
    output sheet when required.

    Args:
        file_path (str): Workbook path.
        source_sheet (str): Source sheet name. Defaults to "Sheet1".
        output_sheet (str): Name for pivot/ETL results. Defaults to "Output".
        transform_type (str): Which transforms to run (e.g. "pivot", "merge").
...
def excel_multi_file(
    ...
    """Build a prompt for multi-file consolidation, validation or comparison.

    The prompt explains how to validate schema compatibility, convert CSVs to
    XLSX when necessary, and run `multi_file` actions such as `aggregate`,
    `filter` or `compare`.

    Args:
        file_paths (str): Comma-separated list of source file paths.
        output_path (str): Destination workbook path for results.
        operation (str): Operation type ("aggregate", "filter", "validate", etc.).
        sheet_name (str): Target sheet name in outputs. Defaults to "Sheet1".
...
def excel_statistical_analysis(
    ...
    """Create a prompt to run regression and smoothing analyses with forecasting.

    The prompt instructs data profiling, running OLS regression and/or
    exponential smoothing, producing interpreted outputs and charts.

    Args:
        file_path (str): Workbook path.
        target_col (str): Column name to predict/forecast.
        sheet_name (str): Sheet name. Defaults to "Sheet1".
        analysis_type (str): "regression", "smoothing", or "both". Defaults to "both".
        feature_cols (str): Comma-separated feature columns for regression.
        forecast_steps (int): Forecast horizon for smoothing methods.
...
def excel_formula_builder(
    ...
    """Return a prompt to write, fill, batch or audit formulas on a sheet.

    The prompt explains the correct usage of `formula_write` and `formula_audit`
    tools (including batch and fill modes) and constraints on formula syntax.

    Args:
        file_path (str): Workbook path.
        sheet_name (str): Sheet name. Defaults to "Sheet1".
        formula_type (str): One of "set", "batch", "fill", "auto_sum" or "audit".
...
def excel_data_governance(
    ...
    """Create a prompt to apply validation, protection, and named-range governance.

    The prompt guides the agent through validation rules, correct protection order
    (unlock editable ranges first), named range creation, and scenario management.

    Args:
        file_path (str): Workbook path.
        sheet_name (str): Sheet name. Defaults to "Sheet1".
        governance_type (str): Which governance actions to apply ("validation","protection","named_ranges","scenarios", or "all").
...
def excel_csv_workflow(
    ...
    """Return a prompt that runs a full CSV-to-Excel workflow and optional export.

    The prompt instructs previewing the CSV, converting to XLSX, cleaning, analysing
    and optionally exporting back to CSV. Emphasises preview and no-inplace edits.

    Args:
        csv_path (str): Read-only source CSV path.
        xlsx_path (str): Destination XLSX path.
        analysis_goal (str): High-level goal (e.g. "clean").
        export_format (str): Output format ("xlsx" or "csv").
...
def excel_readonly_audit(
    ...
    """Build a read-only audit prompt that inspects workbook metadata, data quality and formulas.

    Returns a prompt that instructs the MCP agent to only read metadata and
    summaries (profile, formula inventory, table listing) and to avoid any
    mutating operations.

    Args:
        file_path (str): Workbook path to inspect.
        sheet_name (str): Sheet to inspect. Defaults to "Sheet1".
        scope (str): One of "workbook", "sheet", or "range".
        focus (str): Which aspects to inspect ("metadata","data_quality","formulas","all").
        target_range (str | None): Optional Excel range when scope="range".
...
def excel_formula_diagnosis(
    ...
    """Create a prompt to diagnose (and optionally repair) formula problems.

    The prompt sequences mapping the formula surface, inspecting values,
    precedents and dependents, and (for repair mode) applying minimal safe
    edits via `formula_write`.

    Args:
        file_path (str): Workbook path.
        sheet_name (str): Sheet name. Defaults to "Sheet1".
        target_cells (str | list[str] | None): Target cell(s) to inspect.
        formula_range (str | None): Optional range to bound an errors scan.
        mode (str): One of "diagnose","audit","repair". Defaults to "diagnose".
...
def excel_workbook_maintenance(
    ...
    """Return a prompt to perform conservative workbook maintenance tasks.

    The prompt covers sheet-level management, layout adjustments, print setup and
    row/column operations, with an emphasis on confirmation for destructive ops.

    Args:
        file_path (str): Workbook path.
        sheet_name (str | None): Optional target sheet name(s).
        action (str): Which maintenance action(s) to run ("sheets","layout","print","rows_cols","freeze","all").
...
def excel_table_manager(
    ...
    """Create a prompt to manage native Excel tables: create, resize, totals, or convert.

    The prompt instructs safe table creation, listing, resizing and totals management
    and requires header validation prior to structural edits.

    Args:
        file_path (str): Workbook path.
        sheet_name (str): Sheet name. Defaults to "Sheet1".
        table_name (str | None): Table name for create/resize operations.
        table_range (str | None): Excel range for the table.
        action (str): One of "create","list","data","resize","totals","convert".
        new_range (str | None): New range for resize operations.
        show_totals (bool | None): Whether to show the totals row.
        column_totals (dict[str,str] | None): Per-column total functions.
        style_name (str): Table style to apply. Defaults to "TableStyleMedium9".
...
def excel_what_if_analysis(
    ...
    """Generate a prompt to run goal-seek, solver, sensitivity or scenario analysis.

    The prompt guides input validation, choosing goal-seek vs solver, generating
    sensitivity tables or persisting scenarios, and reporting trade-offs.

    Args:
        file_path (str): Workbook path.
        sheet_name (str): Sheet name. Defaults to "Sheet1".
        analysis_type (str): One of "goal_seek","solver","sensitivity","scenario","all".
        objective_expression (str | None): Optional objective expression text.
        target_value (float | None): Optional numeric target.
        target_cell (str | None): Cell to place the objective.
        variable_cells (str | list[str] | None): Variable cell(s) to vary.
        constraints (str | list[str] | None): Optional bound constraints.
        scenario_name (str | None): Optional scenario name for persistence.
...
def excel_multi_file_reconciliation(
    ...
    """Create a prompt to reconcile or compare data across multiple files.

    The prompt instructs conversion of CSVs if present, schema validation and
    running `multi_file` operations (validate, aggregate, filter, compare).

    Args:
        file_paths (list[str] | str): One or many source file paths.
        output_path (str): Destination path for the reconciliation result.
        operation (str): One of "validate","aggregate","filter","compare".
        sheet_name (str): Sheet name to read from each source. Defaults to "Sheet1".
        key_columns (str | list[str] | None): Optional key column(s) for reconciliation.
...
def excel_search_repair(
    ...
    """Generate a prompt to find and (optionally) repair text or formula content.

    The prompt emphasises read-before-write: preview matches with `read_cells`,
    use `find_replace` for replacements, and re-run formula audits when formulas
    are affected.

    Args:
        file_path (str): Workbook path.
        sheet_name (str): Sheet name. Defaults to "Sheet1".
        search_pattern (str): Text to search for.
        replacement (str): Replacement text. Empty means a search-only preview.
        scope (str): One of "sheet","workbook","formulas".
        match_case (bool): Whether the search is case-sensitive.
        match_entire_cell (bool): Whether the whole cell must match the pattern.
        search_formulas (bool): Whether to search formula text as well as values.
...
def excel_safe_transform(
    ...
    """Create a prompt to apply a sandboxed custom transform or describe it in preview.

    The prompt prefers built-in tools and only permits `execute_custom_code` when
    no safe built-in path exists; it emphasises preview-only workflows and
    writing to `output_path` to avoid overwriting sources.

    Args:
        file_path (str): Source workbook path.
        output_path (str): Destination workbook path.
        sheet_name (str): Sheet to transform. Defaults to "Sheet1".
        transform_goal (str): Short description of the desired transform.
        preview_only (bool): If True, the prompt should describe the transform only.
